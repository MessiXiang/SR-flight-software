...
class Payload02(Node):
    def __init__(self,name):
        ...
        self.add_ints_server_ = self.create_service(Md5ver, "md5ver_srv23", self.hash_calc_md5)
        self.client_ = self.create_client(Md5ver, "md5ver_srv12")
    ...
    def send_request(self):
        if rclpy.ok() and self.client_.wait_for_service(self.max_timeout) == False:
            ...
            if self.no_response_count >= self.restart_count_max:
                ...
                subprocess.Popen("ros2 run restart_node_pkg payload01", shell=True)
                self.no_response_count = 0
            return
